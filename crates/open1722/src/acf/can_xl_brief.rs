...
impl<B: AsRef<[u8]>> CanXlBrief<B> {
    ...
    pub fn acf_msg_length(&self) -> u16 {
        // SAFETY: buffer length validated >= HEADER_LEN at construction.
        unsafe { sys::Avtp_CanXlBrief_GetLen(self.raw()) / sys::AVTP_QUADLET_SIZE as u16 }
    }

    /// Payload length in bytes (excludes header and trailing pad).
    pub fn payload_length(&self) -> u16 {
        // SAFETY: buffer length validated >= HEADER_LEN at construction.
        unsafe { sys::Avtp_CanXlBrief_GetPayloadLen(self.raw()) }
    }

    /// Total message length in bytes (header + payload + pad).
    pub fn message_length(&self) -> u16 {
        // SAFETY: buffer length validated >= HEADER_LEN at construction.
        unsafe { sys::Avtp_CanXlBrief_GetLen(self.raw()) }
    }
...
impl<B: AsRef<[u8]> + AsMut<[u8]>> CanXlBrief<B> {
    ...
    pub fn set_payload_length(&mut self, payload_len: u16) {
        ...
        unsafe { sys::Avtp_CanXlBrief_SetPayloadLen(self.raw_mut(), payload_len) };
    }
}
...
mod tests {
    ...
    fn init_sets_can_xl_brief_acf_type() {
        ...
        assert_eq!(CanXlBrief::new(&buf[..]).unwrap().acf_msg_length(), 4);
    }
